...
class CookPagingSource(
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, Broad> {
        return try {
            ...
            val dataSourceDTO = cookDataSourceApi.searchCookBroadcast(Constants.CLIENT_ID, pageNumber)
            val broadList = dataSourceDTO.broad

            val talkCamList = broadList.filter { broadList ->
                broadList.broadCateNo == categoryNum
            }

            val prevKey = if (pageNumber == STARTING_PAGE_INDEX) null else dataSourceDTO.pageNo - 1
            val nextKey = if (checkEndOfPaginationReached(
                    dataSourceDTO.pageNo,
                    dataSourceDTO.totalCnt,
                    Constants.PAGE_BLOCK
                )
            ) {
                null
            } else {
                dataSourceDTO.pageNo + 1
            }
            LoadResult.Page(
                data = talkCamList,
                prevKey = prevKey,
